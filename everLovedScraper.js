...
export async function scrapeEverLoved(name, location) {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();
  try {
    await page.goto('https://everloved.com/obituary-search/', { waitUntil: 'domcontentloaded' });
    ...
    if (location) {
      await page.fill('input[placeholder="Search by city"]', location);
    }
    // Click the search button if present
    const searchBtn = await page.$('button:has-text("Find an obituary")');
    if (searchBtn) {
      await searchBtn.click();
      await page.waitForLoadState('networkidle');
    } else {
      // fallback: try pressing Enter in the name field
      if (name) await page.keyboard.press('Enter');
      await page.waitForTimeout(2000);
    }

    // Wait for results to load
    await page.waitForSelector('a[href*="/obituaries/"]', { timeout: 10000 });

    // Get up to 10 obituary links
    const obituaryLinks = await page.$$eval('a[href*="/obituaries/"]', links => {
      const seen = new Set();
      return links
    ...
    console.error('Ever Loved scraper error:', err);
  } finally {
    await browser.close();
  }
} 