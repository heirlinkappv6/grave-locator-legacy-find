...
export async function scrapeEchovita(name, location) {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();
  try {
    await page.goto('https://www.echovita.com/us/obituaries', { waitUntil: 'domcontentloaded' });

    // Fill in the search form if present
    if (name) {
      await page.fill('input[placeholder="Who"]', name);
    }
    if (location) {
      await page.fill('input[placeholder="Where"]', location);
    }
    ...
    console.error('Echovita scraper error:', err);
  } finally {
    await browser.close();
  }
} 